...
export const NavbarM = ({ user, logOut }) => {
  ...
  // URL Dinámica para la foto de perfil
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-slate-900/80 backdrop-blur-md border-b border-white/10">
            ...
            <div className="ml-10 flex items-baseline space-x-4">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`px-3 py-2 text-sm font-medium transition-all rounded-lg ${isActive(link.path) ? 'text-white bg-white/10' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                >
                  {link.name}
                </Link>
              ))}
              <Link
                to="/about"
                className={`flex items-center gap-1 px-3 py-2 text-sm font-bold transition-all rounded-lg ${isActive('/about') ? 'text-purple-400 bg-purple-500/10' : 'text-purple-400/80 hover:text-purple-400 hover:bg-purple-500/5'}`}
              >
                <Info size={16} /> About
              </Link>
            </div>
          ...
          <div className="hidden md:flex items-center gap-4">
            {user ? (
              ...
              <div className="flex items-center gap-6">
                ...
                <Link
                  to="/register"
                  className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:opacity-90 transition-all"
                >
                  Join Now
                </Link>
              </div>
      ...
      {isOpen && (
        <div className="lg:hidden bg-slate-900 border-b border-white/10 p-4 space-y-2">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              onClick={() => setIsOpen(false)}
              className={`block px-4 py-3 rounded-xl text-base font-medium ${isActive(link.path) ? 'bg-white/10 text-white' : 'text-gray-400'}`}
            >
              {link.name}
            </Link>
          ))}
